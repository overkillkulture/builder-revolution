...
- **Session revocation — SHIPPED THEN REVERTED (c355ffd):** the per-request `getServerUser` DB-check conflicted with the edge middleware (deleted user with a valid JWT: middleware sends /login→/main on the JWT, page sends /main→/login → redirect loop + 401 flood). Caught via live QA. Reverted to robust JWT-only; revocation is a follow-up needing a loop-free approach (invalidate session on delete, not a per-request check). The other two flagged decisions (bugs-gate, ephemeral guests) stand + verified.
